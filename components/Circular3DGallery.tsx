        ...
        const texture = textureLoader.load(
          ...
          (loadedTexture) => {
            ...
            if (image) {
              ...
              resolve({ texture: loadedTexture, aspectRatio });
            } else {
              // In case the image fails to load
              resolve({ texture: loadedTexture, aspectRatio: 1 });
            }
          },
          undefined,
          () => {
            // Handle errors gracefully
            console.warn(`Failed to load image: assets/img${imageNumber}.jpg`);
            resolve(null);
          }
        );
      });
    ...
    async function initializeBlocks() {
      ...
      for (let section = 0; section < numVerticalSections; section++) {
        ...
        for (let i = 0; i < blocksPerSection; i++) {
          ...
          const imageData = await loadImageTexture(imageNumber);
    
          if (!imageData) continue; // Skip if image fails to load
    
          const { texture, aspectRatio } = imageData;
